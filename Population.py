...
df_urbanPopulation,df_urbanPopTrans = ReadandReturnData(filename,
                                    "Urban population (% of total population)")
df_AgriculturalLand,df_AgriculturalLandTrans = ReadandReturnData(filename,
                                    "Agricultural land (% of land area)")
df_CO2Emissions,df_CO2EmissionsTrans = ReadandReturnData(filename,
                                    "CO2 emissions from solid fuel consumption (% of total)")
df_ArableLand,df_ArableLandTrans = ReadandReturnData(filename,
                                  "Arable land (% of land area)")
df_Population,df_PopulationTrans = ReadandReturnData(filename,
                                 "Population growth (annual %)")


#Calling functions to plot the grpahs
barGraph(df_urbanPopulation, years, 'Country', 'Urban Population Percentage',
         'Urban population (% of total population)')
barGraph(df_AgriculturalLand, years, 'Country', 'Agricultural land Area Percentage',
         'Agricultural land (% of land area)')
linegraph(df_CO2EmissionsTrans, Countries, 'Year', 'CO2 Emission Percentage',
          'CO2 emissions from solid fuel consumption (% of total)')
linegraph(df_ArableLandTrans,Countries, 'Year', 'Arable land Percentage',
          'Arable land (% of land area)')


#Calling function find the mean of population growth
FindMean(df_Population)
